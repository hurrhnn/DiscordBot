...
public class MusicQueueRemoveCommand implements ICmd {
    ...
    public void handle(CmdContext cmdContext) {
        ...
        if (isArgsEmpty(cmdContext.getArgs())) {
            cmdContext.getChannel().sendMessage(EmbedUtils.embedMessageWithTitle("Music - Queue", this.getHelp()).build()).queue();
            return;
        }
        ...
        try {
            ...
            if (arg == 0) {
                cmdContext.getChannel().sendMessage(EmbedUtils.embedMessageWithTitle("Music - Queue", "```E: Please use !!music skip.```").build()).queue();
                return;
            }
            ...
            for (int i = 0; i < musicManager.scheduler.getQueue().size(); i++) {
                ...
                if (arg == (i + 1)) {
                    ...
                    cmdContext.getChannel().sendMessage(EmbedUtils.embedMessageWithTitle("Music - Queue", "Queue number `" + (i + 1) + "` has been removed.").build()).queue();
                    return;
                }
            }
        } catch (NumberFormatException ignored) {
            cmdContext.getChannel().sendMessage(EmbedUtils.embedMessageWithTitle("Music - Queue", "```E: Couldn't remove the queue. Make sure the queue number is correct!```").build()).queue();
        }
    }
